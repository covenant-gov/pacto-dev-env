...
- Bot identities in `pacto-bot-api.toml` (and the `pacto-bot-admin`/generator templates) now list `wss://jskitty.cat/nostr` alongside the local `ws://nostr-relay:8080` relay. `pacto-app` builds that are not running under `tauri:dev` never auto-add the local relay but always connect to `wss://jskitty.cat/nostr` as a trusted default, so this makes bot-published DMs, KeyPackages, and MLS group events visible in any pacto-app build without extra relay setup.
