...
## Trust the local TLS certificate authority

The dev environment uses Caddy to terminate TLS for all Pacto-facing endpoints. Caddy can use locally-trusted certificates generated by `mkcert`, which the setup scripts install. Before starting the services for the first time, run:
